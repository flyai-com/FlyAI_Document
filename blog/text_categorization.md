...
&emsp;&emsp;存放在text文件夹中，其中涉及两个模块：
...
目标：创建sougou.train.txt（训练集11007）、sougou.test.txt（验证集1007）、sougou.val.txt（测试集200*7），其中每一个文件包含每个类下的部分文件，存放于data文件夹中。 
代码实现：
...
> * read_vocab():读取上一步存储的词汇表，转换为{词：id}表示; read_category(): 将分类目录固定，转换为{类别: id}表示;
...
见runCNN.py 若之前进行过训练，请把tensorboard/textcnn删除，避免TensorBoard多次训练结果重叠。
代码实现：
...
> * [利用tensorflow框架实现CNN中文文本分类 ](https://github.com/alicelmx/Chinese-classification#代码实现)
